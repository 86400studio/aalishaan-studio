(()=>{
 ...
 if(form&&data){
  ...
  const savedButton=document.querySelector('[data-saved-toggle]');let savedOnly=false;
  ...
  function update(write=true){
   ...
   savedButton?.setAttribute('aria-pressed',String(savedOnly));chips.replaceChildren();
   for(const [k,v] of Object.entries({...filters,...(savedOnly?{saved:'Saved artworks'}:{})})){if(!v)continue;const b=document.createElement('button');b.type='button';b.textContent=v+' ×';b.setAttribute('aria-label','Remove '+v+' filter');b.onclick=()=>{if(k==='saved')savedOnly=false;else form.elements[k].value='';update();(k==='saved'?savedButton:form.elements[k]).focus();};chips.append(b);}
  ...
  window.addEventListener('wishlistchange',()=>{const removed=savedOnly&&document.activeElement?.matches('[data-wishlist]');update(false);if(removed&&document.activeElement.closest('[hidden]'))savedButton.focus();});window.addEventListener('storage',()=>update(false));
  form.addEventListener('input',()=>update());form.addEventListener('submit',e=>e.preventDefault());form.addEventListener('reset',()=>{savedOnly=false;setTimeout(()=>update(),0);});document.querySelector('[data-reset-filters]').onclick=()=>form.reset();window.addEventListener('popstate',()=>{readURL();update(false);});readURL();update(false);if(matchMedia('(max-width:767px)').matches)document.querySelector('.filters').open=false;
 }
